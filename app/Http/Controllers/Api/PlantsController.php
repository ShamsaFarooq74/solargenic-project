...
class PlantsController extends Controller
{
    ...
    public function allPlants(Request $request)    {
        ...
        //$current_generation = GenerationLog::select(date('H:i',strtotime('created_at')))->whereIn('plant_id',$plant_ids)->whereBetween('created_at', [date('Y-m-d 00:00:00'),date('Y-m-d 23:59:00')])->groupBy('created_at')->get();
        $current_generation1 = GenerationLog::select('cron_job_id')->whereIn('plant_id',$plant_ids)->whereBetween('created_at', [date('Y-m-d 00:00:00'),date('Y-m-d 23:59:00')])->groupBy('cron_job_id')->get();
        //dd($current_generation);
        foreach ($current_generation1 as $key => $today_log) {
            //$today_log_time[] = date('H:i',strtotime($today_log->created_at));
            $today_log_data_sum = GenerationLog::whereIn('plant_id',$plant_ids)->where('cron_job_id',$today_log->cron_job_id)->sum('totalEnergy');
            $today_log_data[] = $key > 0 && $today_log_data_sum <= 0 ? $today_log_data[$key-1] : $today_log_data_sum;
            //$today_log_data[] = $today_log_data_sum;
        }
        $today_log_data[0] = 0;
        //dd($current_generation1, $today_log_data);
        /*$today_log_data1 = [];

        for($i = 0; $i < count($plant_ids); $i++) {

            $today_log_data_sum = GenerationLog::where('plant_id', '=', $plant_ids[$i])->whereBetween('created_at', [date('Y-m-d 00:00:00'),date('Y-m-d 23:59:59')])->pluck('totalEnergy', 'created_at');
            $today_log_data1[] = $today_log_data_sum;
        }

        sort($today_log_data1);

        $merged = array();
        $iteration = 0;

        foreach ($today_log_data1 as $array)
        {
        ...
        //dd($merged);
        $today_log_data = $merged;*/

        $yesterday_log_time=[];$yesterday_log_data=[];
        $yesterday_generation = GenerationLog::select(date('H:i',strtotime('created_at')))->whereIn('plant_id',$plant_ids)->whereBetween('created_at', [date('Y-m-d 00:00:00',strtotime("-1 Days")),date('Y-m-d 23:59:00',strtotime("-1 Days"))])->groupBy('created_at')->get();
        $yesterday_generation1 = GenerationLog::select('cron_job_id')->whereIn('plant_id',$plant_ids)->whereBetween('created_at', [date('Y-m-d 00:00:00',strtotime("-1 Days")),date('Y-m-d 23:59:00',strtotime("-1 Days"))])->groupBy('cron_job_id')->get();
        foreach ($yesterday_generation1 as $key => $yesterday_log) {
            //$yesterday_log_time[] = date('H:i',strtotime($yesterday_log->created_at));
            $yesterday_log_data_sum = GenerationLog::whereIn('plant_id',$plant_ids)->where('cron_job_id',$yesterday_log->cron_job_id)->sum('totalEnergy');
            $yesterday_log_data[] = $key > 0 && $yesterday_log_data_sum <= 0 ? $yesterday_log_data[$key-1] : $yesterday_log_data_sum;
        }
        $yesterday_log_data[0] = 0;
        //dd($today_log_data, $yesterday_log_data);
        /*$yesterday_log_data1 = [];

        for($i = 0; $i < count($plant_ids); $i++) {

            $yesterday_log_data_sum = GenerationLog::where('plant_id', '=', $plant_ids[$i])->whereBetween('created_at', [date('Y-m-d 00:00:00',strtotime("-1 Days")),date('Y-m-d 23:59:00',strtotime("-1 Days"))])->pluck('totalEnergy');
            $yesterday_log_data1[] = $yesterday_log_data_sum;
        }

        sort($yesterday_log_data1);

        $merged1 = array();
        $iteration1 = 0;

        foreach ($yesterday_log_data1 as $array)
        {
            foreach ($array as $key => $value)
            {
                if($iteration1 > 0) {
                    ...
                    if ( ! isset($merged1[$key]))
                    {
                        $merged1[$key] = $merged1[$key-1];
                    }
                    else
                    {
                        $merged1[$key] += $value;
...
//dd($generation_log);
    ...
    public function userPlantDetail($id=0)
    {
        ...
        $page = isset($_GET['page']) ? $_GET['page'] : '';
        if($page == 'refresh'){
            Session::put('plant_idd', $id);
            $controller = new PlantSiteDataController();
            $controller->plant_site_data();
            // app()->call('App\Http\Controllers\Admin\DummyPlantController@plant_site_data');
            $this->get_weather();
        }
    ...
    public function plant_site_data()
    {
        ...
        if(isset($token) && !empty($token)){
           ...
           $curl = curl_init();
            ...
            curl_setopt_array($curl, array(
                CURLOPT_URL => "https://67.23.248.117:8089/api/sites/list?size=1000&startIndex=&sortProperty&sortOrder&isOnline",
                CURLOPT_RETURNTRANSFER => true,
                CURLOPT_CUSTOMREQUEST => "GET",
                CURLOPT_SSL_VERIFYHOST => false,
                CURLOPT_SSL_VERIFYPEER => false,

...
class PlantSiteDataController extends Controller
{
    ...
    public function plant_site_data()
    {
        ...
        $processedCronJobId = ProcessedCurrentVariable::where('processed_cron_job_type','API1')->max('processed_cron_job_id');
        $cronJobTime = new CronJobTime();
        $cronJobTime->start_time = $generation_log_created_time;
        $cronJobTime->type = 'Saltec/Microtech-1';
...
//           $plants = Plant::where('id', 50)->get();
             $plants = Plant::whereIn('meter_type', ['Microtech', 'Saltec','Saltec-Goodwe','Microtech-Goodwe'])->whereBetween('id',[1,320])->get();
//            $plants = Plant::whereIn('meter_type', ['Saltec','Saltec-Goodwe'])->get();

            print_r('Cron job Start Time');
            print_r(date("Y-m-d H:i:s"));
            print_r("\n");
            $generation_log_cron_job_id = GenerationLog::where('cron_job_type','API1')->max('cron_job_id');
            $processed_plant_cron_job_id = ProcessedPlantDetail::where('cron_job_type','API1')->max('cron_job_id');
            if ($plants) {
...
//                                return $site->site_id;
                                ...
                                if (isset($final_processed_data) && $final_processed_data) {
                                    ...
                                    $processed_plant_detail['cron_job_type'] = 'API1';
...
//                                }
                                ...
                                $generation_log['cron_job_type'] = "API1";
                                if ($plant->system_type == 1) {
...
//                    $monthlyGenerationVar = MonthlyInverterDetail::where('plant_id', $plant->id)->whereMonth('created_at', date('m'))->sum('monthly_generation');
                    ...
                    $dailyBoughtEnergy =
                        ProcessedPlantDetail::where('plant_id', $plant->id)->where('cron_job_type','API1')->where('cron_job_id', $processed_plant_cron_job_id + 1)->sum('dailyBoughtEnergy');
                    $dailyConsumptionn =
                        ProcessedPlantDetail::where('plant_id', $plant->id)->where('cron_job_type','API1')->where('cron_job_id', $processed_plant_cron_job_id + 1)->sum('dailyConsumption');
                    $dailySellEnergy =
                        ProcessedPlantDetail::where('plant_id', $plant->id)->where('cron_job_type','API1')->where('cron_job_id', $processed_plant_cron_job_id + 1)->sum('dailySellEnergy');
                    $monthlyBoughtEnergy =
                        ProcessedPlantDetail::where('plant_id', $plant->id)->where('cron_job_type','API1')->where('cron_job_id', $processed_plant_cron_job_id + 1)->sum('monthlyBoughtEnergy');
                    $monthlyConsumptionn =
                        ProcessedPlantDetail::where('plant_id', $plant->id)->where('cron_job_type','API1')->where('cron_job_id', $processed_plant_cron_job_id + 1)->sum('monthlyConsumption');
                    $monthlySellEnergy =
                        ProcessedPlantDetail::where('plant_id', $plant->id)->where('cron_job_type','API1')->where('cron_job_id', $processed_plant_cron_job_id + 1)->sum('monthlySellEnergy');
                    $yearlyBoughtEnergy =
                        ProcessedPlantDetail::where('plant_id', $plant->id)->where('cron_job_type','API1')->where('cron_job_id', $processed_plant_cron_job_id + 1)->sum('yearlyBoughtEnergy');
                    $yearlyConsumptionn =
                        ProcessedPlantDetail::where('plant_id', $plant->id)->where('cron_job_type','API1')->where('cron_job_id', $processed_plant_cron_job_id + 1)->sum('yearlyConsumption');
                    $yearlySellEnergy =
                        ProcessedPlantDetail::where('plant_id', $plant->id)->where('cron_job_type','API1')->where('cron_job_id', $processed_plant_cron_job_id + 1)->sum('yearlySellEnergy');
...
//                    $monthly_Gene_FromAPI = isset($final_processed_data->monthlySolarEnergy) && $final_processed_data->monthlySolarEnergy ? $final_processed_data->monthlySolarEnergy : 0;
            ...
            $plant_data = Plant::whereIn('meter_type', ['Microtech', 'Saltec','Saltec-Goodwe','Microtech-Goodwe'])->whereBetween('id',[1,320])->get(['id', 'plant_name', 'plant_type', 'system_type', 'meter_type', 'meter_serial_no', 'ratio_factor', 'benchmark_price']);
...
//            $plant_data = Plant::where('id',58)->get();
            ...
            $generation_log_cron_job_id2 = GenerationLog::where('cron_job_type','API1')->max('cron_job_id');
            $processed_cron_job_id = ProcessedCurrentVariable::where('processed_cron_job_type','API1')->max('processed_cron_job_id');
            ...
            foreach ($plant_data as $key => $pl_data) {
                ...
                $generation_log_data = GenerationLog::where('plant_id', $pl_data->id)->where('cron_job_type','API1')->where('cron_job_id', $generation_log_cron_job_id2)->get();
...
//                    return $curr_gen;
                    ...
                    $processed_curr_data['processed_cron_job_type'] = 'API1';

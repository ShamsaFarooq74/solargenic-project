...
class DummyPlantController extends Controller
{
                ...
                foreach ($plants as $key => $plant) {
                    ...
                    if($plant_inverter_final_data){
                        ...
                        if($final_processed_data){
                            ...
                            // if($processed_plant_detail_insertion_responce){
                            //     // return true;
                            // }else{
                            //     // return false;
                            // }
    ...
    private function faults_data_insertion($plant_id, $inverters_data, $fault_data,$string,$users){

        $existing_data = FaultAlarmLog::where(['plant_id'=>$plant_id,'fault_and_alarm_id' => $fault_data['id'],'alarm_code' => $fault_data['alarm_code'], 'status' => 'Y'])->orderBy('created_at','desc')->first();
        if($existing_data == null){
            $fault_log['plant_id'] = $plant_id;
            $fault_log['siteId'] = $inverters_data['siteId'];
            $fault_log['fault_and_alarm_id'] = $fault_data['id'];
            $fault_log['type'] = $fault_data['type'];
            $fault_log['severity'] = $fault_data['severity'];
            $fault_log['alarm_code'] = $fault_data['alarm_code'];
            $fault_log['alarm_source'] = $fault_data['alarm_source'];
            $fault_log['status'] = $inverters_data[$string] > 0 ? 'Y' : 'N';
            $fault_log['lastUpdated'] = $inverters_data['lastUpdated'];
            $fault_log['created_at'] = Date('Y-m-d H:i:s',strtotime($inverters_data['lastUpdated']));
            $fault_log['updated_at'] = NUll;
            // dd($fault_log);
            $fault_log_responce = FaultAlarmLog::create($fault_log);

            if(count($users) > 0 && $fault_data['type'] != 'Status'){
    ...
    private function faults_data_updation($plant_id, $fault_data){

        // $existing_data = FaultAlarmLog::where(['plant_id'=>$plant_id,'alarm_code' => $fault_data['alarm_code'],'type' => $fault_data['type'], 'status' => 'Y'])->orderBy('created_at','desc')->first();
        return $fault_data;
        $array = array('plant_id'=> $plant_id,'type' => $fault_data['type'],'alarm_source' => $fault_data['alarm_source'], 'status' => 'Y');
        $existing_data = FaultAlarmLog::where($array)->first();
        // $existing_data = FaultAlarmLog::where(['plant_id'=>$plant_id,'fault_and_alarm_id' => $fault_data['id'],'alarm_code' => $fault_data['alarm_code'], 'status' => 'Y'])->orderBy('created_at','desc')->first();
        // $plant_id == 5 ? dd($fault_data) :'';
        if($existing_data != null){
            $fault_log['status'] = 'N';
            $fault_log['updated_at'] = Date('Y-m-d H:i:s');
            $updation_responce =  $existing_data->fill($fault_log)->save();
        }
        return true;
    }

    private function plant_listing_from_saltec($timestamp)
    {
        $data = [
            'userName' => 'viper.bel',
            'password' => 'vdotb021',
        ...
        if(isset($token) && $token){
            ...
            $curl = curl_init();
            curl_setopt_array($curl, array(
                CURLOPT_URL => "https://67.23.248.117:8089/api/sites/list?size=10&startIndex=0&sortProperty&sortOrder&isOnline",
                CURLOPT_RETURNTRANSFER => true,
                CURLOPT_CUSTOMREQUEST => "GET",
                CURLOPT_SSL_VERIFYHOST => false,
                CURLOPT_SSL_VERIFYPEER => false,
            ...
            // echo '<pre>';print_r($all_plants_data_final);exit;
            date_default_timezone_set("Asia/Karachi");

            foreach ($all_plants_data_final as $key => $plant_data) {
                $plant_input = array(
                    'is_online' => $plant_data->isOnline == (bool)1 ? 'Y' : 'N',
                    'alarmLevel' => $plant_data->alarmLevel,
                    'updated_at' => Date('Y-m-d H:i:s',strtotime($timestamp)),
                );
                $plant_exist = Plant::where('siteId',$plant_data->siteId)->first();
                if($plant_exist){

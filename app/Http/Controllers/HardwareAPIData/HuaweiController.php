...
//        $allPlantsData = Plant::where('id', 105)->where('meter_type', 'Huawei')->get();
        // $allPlantsData = Plant::where('id', 66)->where('meter_type', 'Huawei')->get();
//return json_encode($allPlantsData);
        if ($allPlantsData) {
            ...
            foreach ($allPlantsData as $key => $plant) {
                ...
                if ($plantSiteList) {
                    ...
                    foreach ($plantSiteList as $key1 => $site) {
                        ...
                        if ($siteDevListResponseData->failCode != 305) {
                            ...
                            if (isset($siteDevListFinalData) && $siteDevListFinalData) {
                                ...
                                $countDevList = 0;
                                //	print_r('\n siteDevListFinalData type: '.gettype($siteDevListFinalData));
                                $siteDevListFinalDataArray = json_decode(json_encode($siteDevListFinalData), true);

//return $siteDevListFinalData;
                                foreach ($siteDevListFinalDataArray as $key2 => $dev) {
//                                        return json_encode($dev['devName']);
                                    //	print_r('\n dev: '.json_encode($dev));
...
//                                            ];
//                                            return $siteSmartInverterData;
                                        // return $lastRecordDate;
                                        $type = 1;
                                        $siteSmartInverterResponseData1 = $this->getFiveMinutesData($huaweiAPIBaseURL, $smartInverter, $collectTime, $lastRecordDate, $tokenSessionData[0], $tokenSessionData[1],$type);
//                                        return json_encode($siteSmartInverterResponseData1);
//                                        $siteSmartInverterResponseData = $siteSmartInverterResponseData1[0];
                                        // $siteSmartInverterData = [
                                        //     "devIds" => $smartInverter,
                                        //     "devTypeId" => 1,
                                        //     "collectTime" => $collectTime
...
//                                        $siteSmartInverterResponseData = $siteSmartInverterResponseData1;
                                        // return $siteSmartInverterResponseData22;
                                        $siteSmartInverterResponseData = json_decode($siteSmartInverterResponseData1);
//                                        return $siteSmartInverterResponseData->data;
//                                        $ABC = json_decode($siteSmartInverterResponseData);
//                                        return [json_encode($siteSmartInverterResponseData),gettype($siteSmartInverterResponseData)];
                                        if ($siteSmartInverterResponseData && isset($siteSmartInverterResponseData->data)) {
                                            ...
                                            if ($siteSmartInverterFinalData) {
                                                // return ['hellllllllll' => $siteSmartInverterFinalData];
                                                foreach ((array)$siteSmartInverterFinalData as $smartKeyData => $smartInverterFinalData) {

                                                    // return ['hellllllllll' => $smartInverterFinalData];

                                                    // return $smartInverterFinalData;

                                                    if (isset($smartInverterFinalData->dataItemMap)) {

                                                        $data = $smartInverterFinalData->dataItemMap;
//                                                        return date('Y-m-d H:i:s', substr($smartInverterFinalData->collectTime, 0, -3));
                                                        // return ['hellllllllll' => $data];
                                                        $siteSmartInverterLogStartTime[] = strtotime(date('Y-m-d', substr($smartInverterFinalData->collectTime, 0, -3)));
                                                        $siteAllInverterLogStartTime[] = strtotime(date('Y-m-d', substr($smartInverterFinalData->collectTime, 0, -3)));
                                                        if ($lastRecordDate == date('Y-m-d')) {
                                                            ...
                                                            if (substr($smartInverterFinalData->collectTime, 0, -3) > strtotime($todayLastTime)) {
                                                                ...
                                                                $inverterDetailLog['inverterPower'] = $data->active_power;
                                                                $dailyGenerationData = $data->day_cap != null ? $data->day_cap : $dailyGenerationData;
                                                                $inverterDetailLog['daily_generation'] = $data->day_cap;
                                                                $inverterDetailLog['inverterEfficieny'] = $data->efficiency;
                                                                $inverterDetailLog['inverterTemperature'] = $data->temperature;
                                                                $inverterDetailLog['inverterState'] = $inverterStateString;
                                                                $inverterDetailLog['inverterStateCode'] = $data->inverter_state;
                                                                $inverterDetailLog['mpptPower'] = $data->mppt_power;
                                                                $inverterDetailLog['frequency'] = $data->elec_freq;

                                                                $inverterStartTime = date("Y-m-d H:i:s", $data->open_time);

                                                                $inverterDetailLog['start_time'] = $inverterStartTime;
                                                                $inverterDetailLog['phase_voltage_r'] = $data->a_u;
                                                                $inverterDetailLog['phase_voltage_s'] = $data->b_u;
                                                                $inverterDetailLog['phase_voltage_t'] = $data->c_u;
                                                                $inverterDetailLog['phase_current_r'] = $data->a_i;
                                                                $inverterDetailLog['phase_current_s'] = $data->b_i;
                                                                $inverterDetailLog['phase_current_t'] = $data->c_i;
                                                                $inverterDetailLog['collect_time'] = date('Y-m-d H:i:s', substr($smartInverterFinalData->collectTime, 0, -3));
                                                        ...
                                                        if ($lastRecordDate == date('Y-m-d')) {
                                                            ...
                                                            $todayLastTime = InverterMPPTDetail::where(['plant_id' => $plantID, 'site_id' => $siteID, 'dv_inverter' => $smartInverter])->whereDate('collect_time', $lastRecordDate)->exists() ? InverterMPPTDetail::where(['plant_id' => $plantID, 'site_id' => $siteID, 'dv_inverter' => $smartInverter])->whereDate('collect_time', $lastRecordDate)->orderBy('collect_time', 'desc')->first()->collect_time : date('Y-m-d 00:00:00');
                                                            ...
                                                            if (substr($smartInverterFinalData->collectTime, 0, -3) > strtotime($todayLastTime)) {
                                                                ...
                                                                for ($mi = 1; $mi <= 24; $mi++) {
                                                                    ...
                                                                    $inverterMPPTLog['mppt_voltage'] = $dataInArray['pv' . $mi . '_u'];
                                                                    $inverterMPPTLog['mppt_current'] = $dataInArray['pv' . $mi . '_i'];
                                                                    $inverterMPPTLog['mppt_power'] = (((double)$dataInArray['pv' . $mi . '_u'] * (double)$dataInArray['pv' . $mi . '_i']) / 1000);
                                                                    $inverterMPPTLog['collect_time'] = date('Y-m-d H:i:s', substr($smartInverterFinalData->collectTime, 0, -3));
                                ...
                                if ($plant->plant_has_emi == 'Y') {
                                    ...
                                    foreach ($siteEMIInverterArray as $emiKey => $emiInverter) {
                                        ...
                                        while (strtotime($lastRecordDate) != strtotime(date('Y-m-d', strtotime("+1 days")))) {
                                            ...
                                            $collectTime = strtotime(date('Y-m-d H:i:s.u', strtotime('+5 hours', strtotime($lastRecordDate)))) . '000';
                                            $type = 10;
                                            $siteSmartInverterResponseData1 = $this->getFiveMinutesData($huaweiAPIBaseURL, $emiInverter, $collectTime, $lastRecordDate, $tokenSessionData[0], $tokenSessionData[1],$type);
...
//                                            curl_close($siteEMIInverterCurl);
                                            ...
                                            $siteEMIInverterResponseData = json_decode($siteSmartInverterResponseData1);
                                                ...
                                                if ($siteEMIInverterFinalData) {
                                                    ...
                                                    foreach ((array)$siteEMIInverterFinalData as $emiKeyData => $emiInverterFinalData) {
                                                        ...
                                                        if (isset($emiInverterFinalData->dataItemMap)) {
                                                            ...
                                                            $siteEMIInverterLogStartTime[] = strtotime(date('Y-m-d', substr($emiInverterFinalData->collectTime, 0, -3)));
                                                            $siteAllInverterLogStartTime[] = strtotime(date('Y-m-d', substr($emiInverterFinalData->collectTime, 0, -3)));
                                                            ...
                                                            if ($lastRecordDate == date('Y-m-d')) {
                                                                ...
                                                                if (substr($emiInverterFinalData->collectTime, 0, -3) > strtotime($todayLastTime)) {
                                ...
                                if ($plant->plant_has_grid_meter == 'Y') {
                                    ...
                                    foreach ($siteGridInverterArray as $gridKey => $gridMeter) {
                                        ...
                                        while (strtotime($lastRecordDate) != strtotime(date('Y-m-d', strtotime("+1 days")))) {
                                            ...
                                            $collectTime = strtotime(date('Y-m-d H:i:s.u', strtotime('+5 hours', strtotime($lastRecordDate)))) . '000';
                                            $type = 17;
                                            $siteSmartInverterResponseData1 = $this->getFiveMinutesData($huaweiAPIBaseURL, $gridMeter, $collectTime, $lastRecordDate, $tokenSessionData[0], $tokenSessionData[1],$type);
...
//                                            curl_close($siteInverterEnergyLogCurl);
                                            ...
                                            $siteInverterEnergyLogResponseData = json_decode($siteSmartInverterResponseData1);
                                            ...
                                            if ($siteInverterEnergyLogResponseData && isset($siteInverterEnergyLogResponseData->data)) {
                                                ...
                                                foreach ((array)$siteInverterEnergyLogFinalData as $key6 => $finalData3) {
                                                    ...
                                                    if (isset($finalData3->dataItemMap)) {
                                                        ...
                                                        $siteGridInverterLogStartTime[] = strtotime(date('Y-m-d', substr($finalData3->collectTime, 0, -3)));
                                                        $siteAllInverterLogStartTime[] = strtotime(date('Y-m-d', substr($finalData3->collectTime, 0, -3)));
                                                        ...
                                                        if ($lastRecordDate == date('Y-m-d')) {
                                                            ...
                                                            if (substr($finalData3->collectTime, 0, -3) > strtotime($todayLastTime)) {
...
//                        return date('Y-m-d', min($siteSmartInverterLogStartTime));
...
//                        return ['dummySData' => date('Y-m-d', min($siteEMIInverterLogStartTime))];
                        if ($plant->plant_has_emi == 'Y') {
...
//                return [$siteAllInverterLogStartTime, "dhjsgd"];
...
//        }
    ...
    private function plantStatusUpdate()
    {
        ...
        $plants = DB::table('plants')
            ...
            ->where('plants.meter_type', 'Huawei')
            ->get();
    ...
    public function getFiveMinutesData($huaweiAPIBaseURL, $smartInverter, $collectTime, $lastRecordDate, $tokenSessionData, $tokenSessionData1,$type)
    {
        // return $lastRecordDate;
...
//        try {
        ...
        curl_setopt_array($siteSmartInverterCurl, array(
            ...
            CURLOPT_POSTFIELDS => json_encode($siteSmartInverterData),
            CURLOPT_HTTPHEADER => array(
                'XSRF-TOKEN: ' . $tokenSessionData,
                'Accept: application/json',
                'Content-Type: application/json',
                'Cookie: JSESSIONID=' . $tokenSessionData1 . '; XSRF-TOKEN=eyJhbGciOiJIUzI1NiJ9.eyJyYW5kb21LZXkiOiI3NDNmNDI3ZC1iYWQyLTRhZGMtYWMzZi01N2I5MDg2ZGI5NDEifQ.wXYnbs2FOh_RX_CKlqkjtj6cRDeFc3n4JxXhVLWOFm4; web-auth=true'
            ),
        ));
        ...
        if (isset($siteSmartInverterResponseData->data)) {
            if (count($siteSmartInverterResponseData->data) == 0) {

                $lastRecordDateConvert = date("Y-m-d", strtotime($lastRecordDate . "+1 day"));
                ...
                if (($lastRecordDateConvert <= $todayDate)) {
                    ...
                    sleep(30);
                    $inverterData = self::getFiveMinutesData($huaweiAPIBaseURL, $smartInverter, $collectTimeData, $lastRecordDateConvert, $tokenSessionData, $tokenSessionData1, $type);
                }
            }
        }

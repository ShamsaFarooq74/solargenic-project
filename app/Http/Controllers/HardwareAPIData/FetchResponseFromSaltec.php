...
class FetchResponseFromSaltec extends Controller
{
    ...
    function saltec(Request $request)
    {
        ...
        $cronJobTime->type = 'Saltec/Microtech';
        $cronJobTime->status = 'in-progress';
        $cronJobTime->processed_cron_job_id = $processedCronJobId + 1;
        $cronJobTime->save();
        $all_plants = Plant::whereNotIn('id',[8,48,51,59,63,65,67,69,71,73,85,88,91,98,102,613])->whereIn('meter_type', ['Microtech', 'Saltec', 'Saltec-Goodwe', 'Microtech-Goodwe'])->get();

        foreach ($all_plants as $plants) {

            $plantSites = PlantSite::where('plant_id', $plants->id)->first();
            $data = DB::table('saltec_push_response')->where('site_id', $plantSites->site_id)->where('device_type','!=','')->where('status', "N")->whereDate('collect_time',Date('Y-m-d'))->orderBy('collect_time',"desc")->first();
            ...
            if ($data) {
                ...
                if (isset($final_processed_data) && $final_processed_data) {
                    ...
                    foreach ($final_processed_data as $key => $plant_final_processed_data) {
                        ...
                        $plant = Plant::where('id', $plantSites->plant_id)->first();
                        $benchmark = isset($plants->benchmark_price) ? (int)$plants->benchmark_price : 1;
                        ...
                        foreach ($plant_sites as $key1 => $site) {
                            ...
                            if ($plant_final_processed_data->DeviceType == "MCMT") {
                                ...
                                foreach ($final_processed_data as $key11 => $device_final_processed_data) {
                                    if ($device_final_processed_data->DeviceType == "MPMM") {
                                        $MPPTDeviceData = $device_final_processed_data;
                                    }

                                }
                                // return [$MPPTDeviceData->L1_Frequency_Hz,$MPPTDeviceData];
                                if (isset($plant_final_processed_data->numberOfInverters) && $plant_final_processed_data->numberOfInverters != null && (int)$plant_final_processed_data->numberOfInverters != 0) {
                                    ...
                                    for ($i = 1; $i <= (int)$plant_final_processed_data->numberOfInverters; $i++) {
                                        ...
                                        $inverter['totalGridPower'] = isset($inverters_data['totalGridPower']) ? $inverters_data['totalGridPower'] : null;
                                        ...
                                        $inverter['output_power_l1'] = isset($inverters_data['l1LoadPower']) ? $inverters_data['l1LoadPower'] : null;
                                        $inverter['output_power_l2'] = isset($inverters_data['l2LoadPower']) ? $inverters_data['l2LoadPower'] : null;
                                        $inverter['output_power_l3'] = isset($inverters_data['l3LoadPower']) ? $inverters_data['l3LoadPower'] : null;
                                        $inverter['load_voltage_l1'] = isset($inverters_data['l1Voltage']) ? $inverters_data['l1Voltage'] : null;
                                        $inverter['load_voltage_l2'] = isset($inverters_data['l2Voltage']) ? $inverters_data['l2Voltage'] : null;
                                        $inverter['load_voltage_l3'] = isset($inverters_data['l3Voltage']) ? $inverters_data['l3Voltage'] : null;
                                        $inverter['grid_frequency_l1'] = isset($MPPTDeviceData->L1_Frequency_Hz) ? $MPPTDeviceData->L1_Frequency_Hz : null;
                                        $inverter['grid_frequency_l2'] = isset($MPPTDeviceData->L2_Frequency_Hz) ? $MPPTDeviceData->L2_Frequency_Hz : null;
                                        $inverter['grid_frequency_l3'] = isset($MPPTDeviceData->L3_Frequency_Hz) ? $MPPTDeviceData->L3_Frequency_Hz : null;
                                        $inverter['grid_power_factor_l1'] = isset($inverters_data['l1GridPowerFactor']) ? $inverters_data['l1GridPowerFactor'] : null;
                                        $inverter['grid_power_factor_l2'] = isset($inverters_data['l2GridPowerFactor']) ? $inverters_data['l2GridPowerFactor'] : null;
                                        $inverter['grid_power_factor_l3'] = isset($inverters_data['l3GridPowerFactor']) ? $inverters_data['l3GridPowerFactor'] : null;
                                        $inverter['total_output_power'] = isset($inverters_data['totalLoadPower']) ? $inverters_data['totalLoadPower'] : null;
                                        $inverter['wifi_strength'] = isset($inverters_data['WiFi_Signal_Strength_dBm']) ? $inverters_data['WiFi_Signal_Strength_dBm'] : null;
                                        $inverter['control_mode'] = isset($inverters_data['Control_Mode']) ? $inverters_data['Control_Mode'] : 0;
                                ...
                                // $plantSitesDataPrev = DB::table('saltec_push_response')->where('device_type',"MCMT")->where('site_id',$plantSites->site_id)->whereDate('collect_time',Date("Y-m-d", strtotime("-1 day")))->orderBy('collect_time','desc')->first();
                                ...
                                // }
                                // if(isset($plant_prev_processed_data->solarEnergy) || isset($plant_prev_processed_data->Generated_Energy_kWh) ){
...
//                                            $plant_final_processed_data = "";
                                $salteclatestData = DB::table('saltec_push_response')->where('site_id', $plantSites->site_id)->where('device_type','MCMT')->orderBy('collect_time', "desc")->first();
...
//                                            $curr_gen = 0;
                                            ...
                                            $processed_curr_data['current_saving'] = (double)$dailyGeneration * (double)$plant->benchmark_price;
                                ...
                                if (isset($plant_final_processed_data->Generated_Energy_kWh) || isset($plant_final_processed_data->solarEnergy)) {
                                    ...
                                    $daily_processed['dailySaving'] = (double)$dailyGeneration * $benchmark;
...
//                                    }
                                    ...
                                    //Daily Total Processed Data
                                    $total_daily_generation_exist = SaltecDailyCumulativeData::where('plant_id', $plant->id)->where('site_id',$plantSites->site_id)->whereDate('created_at', Date("Y-m-d", strtotime($plant_final_processed_data->Timestamp)))->first();
                            ...
                            if ($plant_final_processed_data->DeviceType == "MGCE" || $plant_final_processed_data->DeviceType == "MGCW"){
                                $MGCEController = new MGCELoggerController();
                                $MGCEController = $MGCEController->MGCEC($plants,$plant,$site,$benchmark,$plant_final_processed_data,$final_processed_data,$plantSites,$Total_energy_MGCE,$Total_Saving_MGCE,$processed_cron_job_id);
                            //    return  [$MGCEController,$processed_cron_job_id,"MGCE"];
                            }
                        }
                    }
        ...
        $Cumulative_Plants_Data['total_energy'] = $MGCEController[0] + $cumulative_total_energy;
        $Cumulative_Plants_Data['total_saving'] = $MGCEController[1] + $cumulative_total_saving;
